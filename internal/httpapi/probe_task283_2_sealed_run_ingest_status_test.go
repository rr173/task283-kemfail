...
func TestSealedRunIngestMapsConflict(t *testing.T) {
	h := newProbeServer(t)
	runID, paramID, devID := mustProbeRun(t, h)
	req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/api/runs/%d/seal", runID), nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	...
	if rec.Code != http.StatusConflict {
		t.Fatalf("sealed ingest status=%d body=%s", rec.Code, rec.Body.String())
	}
}
